...
#define APP_DELAY                      APP_TIMER_TICKS(2000, APP_TIMER_PRESCALER)
...
static void timers_init(void) {
	...
	ret = app_timer_create(&m_sec_req_timer_id, APP_TIMER_MODE_SINGLE_SHOT,
...
static void on_ancs_c_evt(ble_ancs_c_evt_t * p_evt) {
	...
	case BLE_ANCS_C_EVT_DISCOVERY_COMPLETE:
		NRF_LOG_DEBUG(
				"Apple Notification Center Service discovered on the server.\r\n")
		;
	...
	case BLE_ANCS_C_EVT_DISCOVERY_FAILED:
		NRF_LOG_DEBUG(
				"Apple Notification Center Service not discovered on the server.\r\n")
		;
		break;
...
static void gap_params_init(void) {
	...
	ret = sd_ble_gap_device_name_set(&sec_mode, (const uint8_t *) DEVICE_NAME,
			strlen(DEVICE_NAME));
	APP_ERROR_CHECK(ret);
...
static void on_adv_evt(ble_adv_evt_t ble_adv_evt) {
	...
	case BLE_ADV_EVT_WHITELIST_REQUEST: {
		...
		ret = pm_whitelist_get(whitelist_addrs, &addr_cnt, whitelist_irks,
				&irk_cnt);
		APP_ERROR_CHECK(ret);
		NRF_LOG_DEBUG(
		...
		// Apply the whitelist.
		ret = ble_advertising_whitelist_reply(whitelist_addrs, addr_cnt,
...
static void services_init(void) {
	...
	ret = nrf_ble_ancs_c_attr_add(&m_ancs_c,
			BLE_ANCS_NOTIF_ATTR_ID_APP_IDENTIFIER, m_attr_appid,
			ATTR_DATA_SIZE);
	APP_ERROR_CHECK(ret);

	ret = nrf_ble_ancs_c_app_attr_add(&m_ancs_c,
			BLE_ANCS_APP_ATTR_ID_DISPLAY_NAME, m_attr_disp_name,
	...
	ret = nrf_ble_ancs_c_attr_add(&m_ancs_c,
			BLE_ANCS_NOTIF_ATTR_ID_MESSAGE_SIZE, m_attr_message_size,
			ATTR_DATA_SIZE);
	APP_ERROR_CHECK(ret);
	...
	ret = nrf_ble_ancs_c_attr_add(&m_ancs_c,
			BLE_ANCS_NOTIF_ATTR_ID_POSITIVE_ACTION_LABEL, m_attr_posaction,
			ATTR_DATA_SIZE);
	APP_ERROR_CHECK(ret);

	ret = nrf_ble_ancs_c_attr_add(&m_ancs_c,
			BLE_ANCS_NOTIF_ATTR_ID_NEGATIVE_ACTION_LABEL, m_attr_negaction,
			ATTR_DATA_SIZE);
	APP_ERROR_CHECK(ret);
...
static void advertising_init(void) {
	...
	advdata.uuids_solicited.uuid_cnt = sizeof(m_adv_uuids)
