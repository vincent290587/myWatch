...
#define NRF_LOG_MODULE_NAME "MS5637"
#include "nrf_log.h"
...
bool MS5637::init() {
	...
	if (!this->checkIfPresent(MS5637_ADDR)) {
		return false;
	}

	if (reset() != NRF_SUCCESS)
		return false;
	...
	for (int i = 0; i < 7; i++) {
		...
		if (wireReadDataBlock(CMD_PROM_READ(i), prom_val, 2) != NRF_SUCCESS) {
			err = ERR_BAD_READLEN;
			return false;
		}
...
uint32_t MS5637::takeReading(uint8_t trigger_cmd,
		BaroOversampleLevel oversample_level) {

	if (wireWriteByte(trigger_cmd) != NRF_SUCCESS) {
		return 0;
	}

	uint8_t sampling_delay = pgm_read_byte(
			SamplingDelayMs + (int )oversample_level);
	delay(sampling_delay);

	uint8_t adc[4];
	if (wireReadDataBlock(CMD_READ_ADC, adc, 3) != NRF_SUCCESS) {
		return 0;
	}
